(function() {
    ...
    const domListErrors = document.getElementById("listErrors");
    ...
    const domSectErrors = document.getElementById("sectErrors");
    ...
    function renderDecl(decl_index) {
      ...
      switch (category) {
        ...
        case CAT_error_set: return renderErrorSet(decl_index);
    ...
    function renderErrorSet(decl_index) {
      renderNav(decl_index);
      setViewSourceDecl(decl_index);

      const tokens = errorFields(decl_index);
      if (tokens.length !== 0) {
        resizeDomList(domListErrors, tokens.length, '<li></li>');
        for (let i = 0; i < tokens.length; i += 1) {
          const liDom = domListErrors.children[i];
          liDom.textContent = unwrapToken(decl_index, tokens[i]);

          // underneath the name, we can append a docstring, if it exists
          const docs_html = unwrapString(wasm_exports.error_field_doc(decl_index, tokens[i]));
          if (docs_html.length > 0) {
            liDom.insertAdjacentHTML('beforeend', '<div class="docs">' + docs_html + '</div>');
          }

        }
        domSectErrors.classList.remove("hidden");
      }

      domSourceText.innerHTML = declSourceHtml(decl_index);
      const docs_html = declDocsHtmlShort(decl_index);
      if (docs_html.length > 0) {
        // TODO: apply markdown preprocessing to docs_html
    ...
    function unwrapToken(decl_index, token_index) {
      return unwrapString(wasm_exports.token_name(decl_index, token_index));
    }
    ...
    function errorFields(decl_index) {
      const bigint = wasm_exports.error_fields(decl_index);
      const ptr = Number(bigint & 0xffffffffn);
      const len = Number(bigint >> 32n);
      if (len === 0) return [];
      return new Uint32Array(wasm_exports.memory.buffer, ptr, len);
