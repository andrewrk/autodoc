(function() {
    ...
    function renderErrorSet(decl_index) {
      ...
      if (tokens.length !== 0) {
        resizeDomList(domListFnErrors, tokens.length, '<li></li>');
        for (let i = 0; i < tokens.length; i += 1) {
          const token = tokens[i];
          const liDom = domListFnErrors.children[i];
          liDom.textContent = identString(decl_index, token);

          // underneath the name, we can append a docstring, if it exists
          const docs_html = errorFieldDoc(decl_index, token);
    ...
    function errorFieldDoc(decl_index, token_index) {
      return unwrapString(wasm_exports.error_field_doc(decl_index, token_index));
    }

    function identString(decl_index, token_index) {
      return unwrapString(wasm_exports.identifier_string(decl_index, token_index));
    }
